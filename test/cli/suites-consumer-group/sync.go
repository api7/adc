...
var _ = ginkgo.Describe("`adc sync` tests", func() {
	...
	ginkgo.Context("Basic functions", func() {
		...
		ginkgo.It("should sync data to APISIX", func() {
			...
			_, err := s.UpdateConsumerGroup(consumerGroup)
			gomega.Expect(err).To(gomega.BeNil(), "check consumerGroup update")
			...
			err = s.DeleteConsumerGroup(id)
			gomega.Expect(err).To(gomega.BeNil(), "check consumerGroup delete")
