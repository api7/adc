...
var _ = ginkgo.Describe("`adc sync` tests", func() {
	...
	ginkgo.Context("Basic functions", func() {
		...
		ginkgo.It("should sync data to APISIX", func() {
			...
			_, err := s.UpdatePluginConfig(pluginConfig)
			gomega.Expect(err).To(gomega.BeNil(), "check pluginConfig update")
			...
			err = s.DeletePluginConfig(id)
			gomega.Expect(err).To(gomega.BeNil(), "check pluginConfig delete")
