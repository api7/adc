...
async fn syncs_and_dumps_services_lifecycle() {
    ...
    sync_ok(
        &backend,
        vec![
            create(ResourceType::Service, "service1", json!({ "name": "service1", "upstream": upstream, "hosts": ["example1.com", "example2.com"] })),
            create(ResourceType::Service, "service2", json!({ "name": "service2", "upstream": upstream })),
        ],
    )
    .await;
    ...
    let service1 = services.iter().find(|s| s.name == "service1").expect("service1 missing");
    ...
    sync_ok(
        ...
        vec![update(
            ...
            "service1",
            json!({ "name": "service1", "upstream": upstream, "hosts": ["example1.com", "example2.com"] }),
            json!({ "name": "service1", "upstream": upstream, "hosts": ["example1.com", "example2.com"], "description": "desc" }),
        )],
    )
    .await;

    let config = dump(&backend).await;
    let service1 = config.services.unwrap().into_iter().find(|s| s.name == "service1").expect("service1 missing");
    assert_eq!(service1.description.as_deref(), Some("desc"));

    sync_ok(&backend, vec![delete(ResourceType::Service, "service1")]).await;
    let config = dump(&backend).await;
    let services = config.services.unwrap();
    assert_eq!(services.len(), 1);
    assert_eq!(services[0].name, "service2");

    sync_ok(&backend, vec![delete(ResourceType::Service, "service2")]).await;
    let config = dump(&backend).await;
...
async fn syncs_and_dumps_service_with_routes_lifecycle() {
    ...
    sync_ok(
        &backend,
        vec![
            create(ResourceType::Service, "test", json!({ "name": "test", "upstream": upstream })),
            create_child(ResourceType::Route, "route1", json!({ "name": "route1", "uris": ["/route1"] }), "test"),
            create_child(ResourceType::Route, "route2", json!({ "name": "route2", "uris": ["/route2"], "plugins": { "key-auth": {} } }), "test"),
        ],
    )
    .await;
    ...
    assert!(routes.iter().any(|r| r.name == "route1" && r.uris == vec!["/route1".to_string()]));
    assert!(routes.iter().any(|r| r.name == "route2" && r.plugins.is_some()));

    sync_ok(&backend, vec![delete_child(ResourceType::Route, "route1", "test")]).await;
    let config = dump(&backend).await;
    let services = config.services.unwrap();
    let routes = services[0].routes.as_ref().unwrap().http().unwrap();
    assert_eq!(routes.len(), 1);
    assert_eq!(routes[0].name, "route2");

    sync_ok(&backend, vec![delete_child(ResourceType::Route, "route2", "test")]).await;
    ...
    sync_ok(&backend, vec![delete(ResourceType::Service, "test")]).await;
...
async fn syncs_and_dumps_service_with_stream_route_lifecycle() {
    ...
    sync_ok(
        ...
        vec![
            create(ResourceType::Service, "test", json!({ "name": "test", "upstream": upstream })),
            create_child(ResourceType::StreamRoute, "postgres", json!({ "name": "postgres", "server_port": 54320 }), "test"),
        ],
    )
    .await;
    ...
    if apisix_version() >= semver::Version::new(3, 8, 0) {
        // Below 3.8.0 APISIX doesn't support labels on stream routes, so
        // the `__ADC_NAME` trick can't recover the original name — the
        // fetcher falls back to the id in that case (see `transformer.rs`).
        assert_eq!(stream_routes[0].name, "postgres");
    }

    sync_ok(&backend, vec![delete_child(ResourceType::StreamRoute, "postgres", "test")]).await;
    let config = dump(&backend).await;
    let services = config.services.unwrap();
...
async fn syncs_and_dumps_consumers_lifecycle() {
    ...
    sync_ok(
        &backend,
        vec![
            create(ResourceType::Consumer, "consumer1", json!({ "username": "consumer1", "plugins": { "key-auth": { "key": "consumer1" } } })),
            create(ResourceType::Consumer, "consumer2", json!({ "username": "consumer2", "plugins": { "key-auth": { "key": "consumer2" } } })),
        ],
    )
    .await;
    ...
    assert!(consumers.iter().any(|c| c.username == "consumer1"));
    assert!(consumers.iter().any(|c| c.username == "consumer2"));
    ...
    sync_ok(
        ...
        vec![update(
            ...
            "consumer1",
            json!({ "username": "consumer1", "plugins": { "key-auth": { "key": "consumer1" } } }),
            json!({ "username": "consumer1", "plugins": { "key-auth": { "key": "consumer1" } }, "description": "desc" }),
        )],
    )
    .await;
    let config = dump(&backend).await;
    let consumer1 = config.consumers.unwrap().into_iter().find(|c| c.username == "consumer1").unwrap();
    assert_eq!(consumer1.description.as_deref(), Some("desc"));

    sync_ok(&backend, vec![delete(ResourceType::Consumer, "consumer1")]).await;
    let config = dump(&backend).await;
    let consumers = config.consumers.unwrap();
    assert_eq!(consumers.len(), 1);
    assert_eq!(consumers[0].username, "consumer2");

    sync_ok(&backend, vec![delete(ResourceType::Consumer, "consumer2")]).await;
    let config = dump(&backend).await;
...
async fn syncs_and_dumps_global_rules_lifecycle() {
    ...
    sync_ok(
        &backend,
        vec![
            create(ResourceType::GlobalRule, "prometheus", json!({ "prefer_name": true })),
            create(ResourceType::GlobalRule, "file-logger", json!({ "path": "logs/file.log" })),
        ],
    )
    .await;
    ...
    assert_eq!(rules.get("prometheus").and_then(|v| v.get("prefer_name")), Some(&json!(true)));

    sync_ok(
        &backend,
        vec![update(ResourceType::GlobalRule, "prometheus", json!({ "prefer_name": true }), json!({ "prefer_name": true, "test": "test" }))],
    )
    .await;
    let config = dump(&backend).await;
    assert_eq!(config.global_rules.unwrap().get("prometheus").and_then(|v| v.get("test")), Some(&json!("test")));

    sync_ok(&backend, vec![delete(ResourceType::GlobalRule, "prometheus")]).await;
    let config = dump(&backend).await;
    let rules = config.global_rules.unwrap();
    assert_eq!(rules.len(), 1);
    assert!(rules.contains_key("file-logger"));

    sync_ok(&backend, vec![delete(ResourceType::GlobalRule, "file-logger")]).await;
    let config = dump(&backend).await;
...
async fn syncs_and_dumps_plugin_metadata_lifecycle() {
    ...
    sync_ok(
        &backend,
        vec![
            create(ResourceType::PluginMetadata, "http-logger", json!({ "log_format": { "test": "test", "test1": "test1" } })),
            create(ResourceType::PluginMetadata, "tcp-logger", json!({ "log_format": { "test": "test", "test1": "test1" } })),
        ],
    )
    .await;
    ...
    assert_eq!(metadata.get("http-logger").and_then(|v| v.get("log_format")).and_then(|v| v.get("test")), Some(&json!("test")));
    ...
    .await;
    let config = dump(&backend).await;
    assert_eq!(config.plugin_metadata.unwrap().get("http-logger").and_then(|v| v.get("test")), Some(&json!({ "value": "test" })));

    sync_ok(&backend, vec![delete(ResourceType::PluginMetadata, "http-logger")]).await;
    let config = dump(&backend).await;
    let metadata = config.plugin_metadata.unwrap();
    assert_eq!(metadata.len(), 1);
    assert!(metadata.contains_key("tcp-logger"));

    sync_ok(&backend, vec![delete(ResourceType::PluginMetadata, "tcp-logger")]).await;
    let config = dump(&backend).await;
