/**
 * Unit tests for the API7 EE Validator.
 *
 * NOTE: These tests are fully mocked — they test that the Validator correctly
 * maps API7 validate errors back to ADC Event objects by asserting on the
...
describe('API7 Validator', () => {
  ...
  it('should throw when version is below minimum', async () => {
    const events = [createEvent(ADCSDK.ResourceType.SERVICE, 'my-service')];
    ...
    const validator = new Validator({
      client,
      eventSubject: new Subject<ADCSDK.BackendEvent>(),
      version: new SemVer('3.9.9'),
      gatewayGroupId: 'default',
    });

    await expect(validator.validate(events)).rejects.toThrow(
      'Validate is not supported by the current backend version',
    );
  });
});
