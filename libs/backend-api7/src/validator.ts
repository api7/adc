...
export class Validator extends ADCSDK.backend.BackendEventSource {
  ...
  public async validate(
    ...
    if (semverLT(this.opts.version, MINIMUM_VALIDATE_VERSION))
      throw new Error(
        `Validate is not supported by the current backend version (${this.opts.version}). Please upgrade to a newer version.`,
      );
  ...
  private buildRequestBody(events: Array<ADCSDK.Event>): {
    ...
    const nameIndex = structuredClone(body) as unknown as Record<
      string,
      string[]
    >;
    const eventIndex = structuredClone(body) as unknown as Record<
      string,
      Array<ADCSDK.Event | undefined>
    >;
